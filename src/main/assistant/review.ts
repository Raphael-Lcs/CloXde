  ...
  try {
    ...
    const intro = snapshot.stuck
      ? snapshot.exhausted
        ...
        : '⚠️ 注意：下面带 ⚠️ 的团队已经**卡住**、在等你决策。\n\n**重要**：自动模式（autopilot）下，团队状态 awaiting-user 意味着**引擎遇到协议违例、崩溃或受阻而暂停**，**不是团队完成了工作**！团队完成时状态应该是 "ended"，而不是 "awaiting-user"。\n\n你的任务：用 <<CONTINUE>> 给团队补充指示、纠偏、回答它抛出的问题，把它推回正轨继续执行。'
...
【如何推动卡住的团队】
- **必须发标签**：光说"我推动团队"没用，**必须真的发 <<CONTINUE>>** 标签系统才会执行
- 标签格式：<<CONTINUE>>{"conversationId":"会话ID","message":"你要对团队说的话"}<</CONTINUE>>
- message 内容：分析团队卡在哪、需要什么，给出明确指示（补充要求、纠正方向、回答问题）
- **标签在哪都行**：可以在回复文字前、中、后任何位置，但**必须发标签**才会真正执行
